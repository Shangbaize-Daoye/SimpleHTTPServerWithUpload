...
class SimpleHTTPRequestHandler(http.server.BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        f.write(info.encode(enc))
        f.write(
            ("<br><br><a href=\"%s\">" % self.headers['referer']).encode(enc))
        f.write(b"<button>Back</button></a>\n")
        f.write((
            "<hr><small>Powered by: Zeyu丶泽宇<br>Contact with Zeyu丶泽宇: zhang@zeyu.tw"
        ).encode(enc))
    ...
    def list_directory(self, path):
        ...
        f.write(b'a { text-decoration: none; }\n')
        f.write(
            b'a:link { text-decoration: none; font-weight: bold; color: #0000ff; }\n'
        )
        f.write(
            b'a:visited { text-decoration: none; font-weight: bold; color: #0000ff; }\n'
        )
        f.write(
            b'a:active { text-decoration: none; font-weight: bold; color: #0000ff; }\n'
        )
        f.write(
            b'a:hover { text-decoration: none; font-weight: bold; color: #ff0000; }\n'
        )
        ...
        f.write(b'<table>\n')
        f.write(
            b'<tr><td><img src="data:image/gif;base64," alt="[PARENTDIR]" width="24" height="24"></td><td><a href="../" >Parent Directory</a></td></tr>\n'
        )
        for name in list:
            ...
            f.write((
                ...
                % (dirimage, urllib.parse.quote(linkname),
                   html.escape(displayname), fsize, created_date)).encode(enc))
        f.write(b"</table><hr>\n</body>\n</html>\n")
        length = f.tell()
        f.seek(0)
        self.send_response(200)
